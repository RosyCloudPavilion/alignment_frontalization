...
class Alignmenter(object):
    ...
    def align(self,dir_save_path,time_stemp):
        ...
        #获取每个UAV传来的文件夹位置，UAV_paths_all包含每个摄像头的字典，字典的key是时间戳，值是时间戳对应的数据，UAV_num为摄像头的数目
        UAV_paths_all,UAV_num=self.load_UAV_dir(dir_save_path)

        key = str(time_stemp)
        #获取当前时间戳中每个摄像头的数据地址，bb_paths即为每个摄像头当前时间戳的数据地址的列表
        bb_paths=[]
        for i in range(UAV_num):
            bb_paths.append(UAV_paths_all[i][key])
        #print("time_stemp:",key)

        #根据数据地址加载数据，boxes_allview为各摄像头当前帧的检测框列表，image_allview为各摄像头当前帧的图像list，imgnum为图片数量（理论上会等于摄像头的数量）
